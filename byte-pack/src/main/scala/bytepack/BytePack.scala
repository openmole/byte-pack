...
//    fillBytes(4, IArray.unsafeFromArray(buff.array()))
  ...
  inline def extractFloat(inline b: IArray[Byte], inline index: Int = 0) = java.lang.Float.intBitsToFloat(extractInt(b, index))
  inline def extractDouble(inline b: IArray[Byte], inline index: Int = 0) = 
    ByteBuffer.wrap(Array[Byte](b(index), b(index + 1), b(index + 2), b(index + 3), b(index + 4), b(index + 5), b(index + 6), b(index + 7))).getDouble
